...
`output_refined.mp3` is the output music for *La Luna*, converted to mp3 format using CoolSoft VirtualMidiSynth (2.6.0) and sound font Merlin Creative.    
...
The raw `output.mid` and `no_transition.mid` as would be generated by the code with the sample input are also found there.    

Sample transition output for different types of transition between two specific music segments can be found in the presentation slides.    
